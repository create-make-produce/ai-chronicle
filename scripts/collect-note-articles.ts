...
async function fetchNoteArticles(sort: 'popular' | 'hot', limit: number): Promise<NoteArticle[]> {
  ...
  return notes.slice(0, limit).map((n) => ({
    ...
    author_icon_url: n.user?.user_profile_image_path ?? null,
...
async function saveArticleForTool(db: D1Client, toolId: string, article: NoteArticle): Promise<boolean> {
  const existing = await db.first<{ id: string }>(`SELECT id FROM tool_note_articles WHERE note_url = ? AND tool_id = ? LIMIT 1`, [article.note_url, toolId]);
...
async function main() {
  ...
  await db.execute(`INSERT INTO scrape_logs (id, job_name, status, started_at) VALUES (?, 'collect_note_popular', 'running', datetime('now'))`, [logId]);

  let saved = 0, skipped = 0;
  try {
    const articles = await fetchNoteArticles('popular', FETCH_LIMIT);
    if (articles.length === 0) {
      console.log('  ⚠️ 0件 → note.com APIの仕様変更の可能性');
