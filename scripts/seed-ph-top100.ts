  ...
  try {
    ...
    await db.execute(
      ...
      [
        ...
        post.product_id ?? post.id,         // product_hunt_id（製品ID優先）
        post.product_url ?? post.url,       // product_hunt_url（製品ページ優先）
...
async function main() {
  console.log('╔══════════════════════════════════════╗');
  console.log('║  AI Chronicle - PH Top 100 シード    ║');
  console.log('╚══════════════════════════════════════╝');
  console.log('⚠️  categories以外のDBデータを全削除します\n');
  ...
  // STEP 1: PH Top 100取得
  console.log('📡 Product Hunt AI Top 100 取得中...');
  let posts: ProductHuntPost[];
  try {
    posts = await fetchTopAIPosts(100);
  } catch (e) {
