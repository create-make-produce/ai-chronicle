...
{
  "title_ja": "日本語タイトル（30文字以内・【重要】ツール名・製品名は絶対に含めない・何ができるかを具体的に書き「〜が提供開始」で締める。例：「会話内容を外部送信しないプライバシーAIが提供開始」）",
  "title_en": "English title (under 60 characters)",
  "body_ja": "日本語本文（日本のニュース記事のような文体で書く。難しい専門用語・カタカナ語は使わない（「ウェルビーイング」「ソリューション」「インサイト」等は日本語に言い換える）。何が便利かが分かりやすい文章にする。何ができるツールか・特徴・想定ユーザーを含む。文字数：300〜1000文字。「。」は全体で3〜4個以内に抑える。「。」の後は必ず改行を入れる。複数の内容は必ず「、」でつないで1文にまとめること。「〜します。〜です。〜ます。」のように短文を並べることは絶対禁止。本文中にURLを記載しないこと）",
...
{
  "title_ja": "日本語タイトル（30文字以内・【重要】ツール名・製品名は絶対に含めない・「追加・強化された機能名」＋「それによって何ができるようになったか」を書く。同じ内容の言い換えは禁止。例：「オフラインモードを追加、インターネット不要で音声処理が可能に」「リアルタイム翻訳機能を追加、65言語に対応」）",
  "title_en": "English title (under 60 characters)",
  "body_ja": "日本語本文（日本のニュース記事のような文体で書く。難しい専門用語・カタカナ語は使わない（「ウェルビーイング」「ソリューション」「インサイト」等は日本語に言い換える）。何が便利かが分かりやすい文章にする。どんな機能か・誰に役立つか・使い方を含む。文字数：300〜1000文字。「。」は全体で3〜4個以内に抑える。「。」の後は必ず改行を入れる。複数の内容は必ず「、」でつないで1文にまとめること。「〜します。〜です。〜ます。」のように短文を並べることは絶対禁止。本文中にURLを記載しないこと）",
...
{
  "title_ja": "日本語タイトル（30文字以内・【重要】ツール名・製品名は絶対に含めない・何がどう変わったかを書く。例：「無料プランが廃止、全プラン有料化へ」「月額プランが値上げ、新料金は来月から適用」）",
  "title_en": "English title",
  "body_ja": "日本語本文（日本のニュース記事のような文体で書く。難しい専門用語・カタカナ語は使わない（「ウェルビーイング」「ソリューション」「インサイト」等は日本語に言い換える）。何が便利かが分かりやすい文章にする。何がいつから変わるか・影響するユーザーを含む。文字数：300〜1000文字。「。」は全体で3〜4個以内に抑える。「。」の後は必ず改行を入れる。複数の内容は必ず「、」でつないで1文にまとめること。「〜します。〜です。〜ます。」のように短文を並べることは絶対禁止。本文中にURLを記載しないこと）",
  "body_en": "English body (10-15 lines)"
}`;
...
async function generatePriceChangeLaunchNews(event: Extract<NewsEvent, { type: 'price_change_launch' }>): Promise<{
  title_ja: string; title_en: string; body_ja: string; body_en: string;
}> {
  const { tool, launch } = event;
  const tagline = launch.tagline_ja ?? launch.tagline ?? '';

  const prompt = `以下のAIツールの価格改定についてニュース記事を日本語と英語で生成してください。

ツール情報：
- ツール名（日）: ${tool.name_ja}
- ツール名（英）: ${tool.name_en}
- 改定内容: ${launch.launch_name}
- 詳細: ${tagline}
...

{
  "title_ja": "日本語タイトル（30文字以内・【重要】ツール名・製品名は絶対に含めない・何がどう変わったかを書く。例：「無料プランが廃止、全プラン有料化へ」「月額プランが値上げ、新料金は来月から適用」）",
  "title_en": "English title (under 60 characters)",
  "body_ja": "日本語本文（日本のニュース記事のような文体で書く。難しい専門用語・カタカナ語は使わない（「ウェルビーイング」「ソリューション」「インサイト」等は日本語に言い換える）。何が便利かが分かりやすい文章にする。何がどう変わるか・影響するユーザーを含む。文字数：300〜1000文字。「。」は全体で3〜4個以内に抑える。「。」の後は必ず改行を入れる。複数の内容は必ず「、」でつないで1文にまとめること。「〜します。〜です。〜ます。」のように短文を並べることは絶対禁止。本文中にURLを記載しないこと）",
  "body_en": "English body (10-15 lines)"
}`;
...

{
  ...
  "title_en": "English title (under 60 characters)",
  "body_ja": "日本語本文（日本のニュース記事のような文体で書く。難しい専門用語・カタカナ語は使わない（「ウェルビーイング」「ソリューション」「インサイト」等は日本語に言い換える）。何が便利かが分かりやすい文章にする。何が発表されたか・背景・影響を含む。文字数：300〜1000文字。「。」は全体で3〜4個以内に抑える。「。」の後は必ず改行を入れる。複数の内容は必ず「、」でつないで1文にまとめること。「〜します。〜です。〜ます。」のように短文を並べることは絶対禁止。本文中にURLを記載しないこと）",
  "body_en": "English body (10-15 lines)"
}`;
