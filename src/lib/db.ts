...
export async function getPublishedTools(limit = 30, offset = 0): Promise<Tool[]>{
    return queryD1<Tool>(
      `SELECT * FROM tools WHERE is_published = 1 AND status = 'active' ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      [limit, offset],
    );
}
...
export async function getToolCount(): Promise<number>{
    const rows = await queryD1<{ c: number }>(
      `SELECT COUNT(*) AS c FROM tools WHERE is_published = 1 AND status = 'active'`,
    );
    return rows[0]?.c ?? 0;
}
...
export async function getToolBySlug(slug: string): Promise<Tool | null>{
    const rows = await queryD1<Tool>(
      `SELECT * FROM tools WHERE slug = ? AND is_published = 1 AND status != 'archived' LIMIT 1`,
      [slug],
    );
    return rows[0] ?? null;
}
...
export async function getAllToolSlugs(): Promise<string[]>{
    const rows = await queryD1<{ slug: string }>(
      `SELECT slug FROM tools WHERE is_published = 1 AND status = 'active'`,
    );
...
export async function getToolById(id: string): Promise<Tool | null>{
    const rows = await queryD1<Tool>(
      `SELECT * FROM tools WHERE id = ? AND is_published = 1 AND status != 'archived' LIMIT 1`,
      [id],
    );
    return rows[0] ?? null;
}
...
export async function getToolsByCategory(categorySlug: string, limit = 30, offset = 0): Promise<Tool[]>{
    return queryD1<Tool>(
      `SELECT t.* FROM tools t
       ...
       WHERE c.slug = ? AND t.is_published = 1 AND t.status = 'active'
...
export async function getNewToolsSince(hoursAgo: number, limit = 10): Promise<Tool[]>{
    ...
    return queryD1<Tool>(
      `SELECT * FROM tools
       WHERE is_published = 1 AND status = 'active' AND created_at >= ?
...
export async function getFreeTools(limit = 12): Promise<Tool[]>{
    return queryD1<Tool>(
      `SELECT * FROM tools
       WHERE is_published = 1 AND status = 'active' AND has_free_plan = 1
       ORDER BY created_at DESC
       LIMIT ?`,
      [limit],
    );
...
export async function getRelatedTools(categoryId: string | null, excludeId: string, limit = 5): Promise<Tool[]>{
    ...
    return queryD1<Tool>(
      `SELECT * FROM tools
       WHERE is_published = 1 AND status = 'active' AND category_id = ? AND id != ?
       ORDER BY created_at DESC
       LIMIT ?`,
...
export async function getRecentPriceChanges(days: number, limit = 5){
    ...
    >(
      `SELECT p.*, t.name_ja AS tool_name_ja, t.name_en AS tool_name_en, t.slug AS tool_slug
       ...
       WHERE t.is_published = 1 AND t.status = 'active'
...
export async function getCategoriesWithCount(): Promise<CategoryWithCount[]>{
    return queryD1<CategoryWithCount>(
      `SELECT c.*, COUNT(t.id) AS tool_count
       FROM categories c
       LEFT JOIN tools t ON t.category_id = c.id AND t.is_published = 1 AND t.status = 'active'
       GROUP BY c.id
    ...
    return queryD1<News>(
      `SELECT n.*, t.name_ja as tool_name_ja, t.name_en as tool_name_en, t.logo_url as tool_logo_url
       ...
       WHERE n.is_published = 1
         AND (n.tool_id IS NULL OR (t.is_published = 1 AND t.status != 'archived'))
