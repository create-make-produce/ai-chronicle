...
function formatMonthEn(ym: string) {
  const [y, m] = ym.split('-');
  const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  return `${months[parseInt(m, 10) - 1]} ${y}`;
}

export default async function MonthlyPageEn({ searchParams }: { searchParams: Promise<{ m?: string }> }) {
    ...
    <main style={{ minHeight: '100vh', background: '#111318' }}>
      <section style={{ background: 'linear-gradient(135deg, #0A0D12 0%, #111827 100%)', borderBottom: '1px solid rgba(0,140,237,0.15)', padding: '2rem 1.5rem 2.5rem' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.9rem', color: '#7A8A99' }}>
            Monthly roundup of updated AI tools
          </p>
        </div>
      </section>
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          {prevMonth && (
            <Link href={`/en/monthly?m=${prevMonth}`} style={{ fontFamily: 'Fira Sans, sans-serif', fontSize: '0.82rem', color: '#7A8A99', textDecoration: 'none', padding: '6px 12px', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px' }}>
              ← {formatMonthEn(prevMonth)}
            </Link>
          )}
          <span style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '1.1rem', fontWeight: 700, color: '#F0EBE1', padding: '6px 18px', background: 'rgba(0,140,237,0.12)', border: '1px solid rgba(0,140,237,0.3)', borderRadius: '4px' }}>
            {formatMonthEn(selectedMonth)}
          </span>
          {nextMonth && (
            <Link href={`/en/monthly?m=${nextMonth}`} style={{ fontFamily: 'Fira Sans, sans-serif', fontSize: '0.82rem', color: '#7A8A99', textDecoration: 'none', padding: '6px 12px', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px' }}>
              {formatMonthEn(nextMonth)} →
            </Link>
          )}
        ...
        <MonthSelect months={availableMonths} selectedMonth={selectedMonth} basePath="/en/monthly" lang="en" />

        {tools.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#4A5568', fontFamily: 'Inter, sans-serif', fontSize: '0.9rem' }}>
            No updates for {formatMonthEn(selectedMonth)} yet.
          </div>
        ) : (
